...
const ITEMS = ['Dell Laptop Pro 15', 'HP LaserJet Printer', 'Office Chair Ergonomic', 'A4 Paper (500 reams)', 'Lab Microscope', 'Network Switch 48-Port', 'Air Conditioner 1.5T', 'Projector Epson'];

export default function PurchaseRequest() {
    const navigate = useNavigate();
    const [step, setStep] = useState(0);
    const [form, setForm] = useState({ dept: '', item: '', qty: 1, unitCost: '', justification: '', priority: 'Medium' });
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="bg-zinc-900/70 border border-zinc-800/80 rounded-2xl p-6 space-y-4">
                {step === 0 && (
                    <>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                            {['Administration', 'Finance', 'IT', 'Science', 'Library', 'Security'].map(d => (
                ...
                {step === 2 && (
                    <>
                        ...
                        <div>
                            <label className="text-slate-300 text-xs font-medium mb-1.5 block">Justification</label>
                ...
                {step === 3 && (
                    <>
                        ...
                        <div className="bg-[#0e0e11]/40 border border-zinc-800 rounded-xl p-4 space-y-2.5">
                            ...
                            {[['Department', form.dept], ['Item', form.item], ['Quantity', form.qty], ['Unit Cost', `₹${parseFloat(form.unitCost || 0).toLocaleString()}`], ['Total', `₹${total.toLocaleString()}`], ['Priority', form.priority]].map(([k, v]) => (
            ...
            <div className="flex justify-between">
                ...
                <button onClick={() => step < STEPS.length - 1 ? setStep(s => s + 1) : navigate('/procurement/approvals')}
                    ...
                    {step === STEPS.length - 1 ? 'Submit Request' : 'Continue'} {step < STEPS.length - 1 && <ChevronRight size={16} />}
                </button>
            </div>
        </div>
    );
