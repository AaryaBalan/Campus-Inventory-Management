...
import {
    ...
    ChevronRight, BarChart3, Filter, Monitor
} from 'lucide-react';
            ...
            <div className="relative z-50 w-full sm:max-w-3xl max-h-[90vh] sm:max-h-[85vh]
                ...
                <div className="flex-1 overflow-y-auto px-10 pb-10 space-y-7">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        {[
                            { icon: <Package size={15} />, label: 'Total Assets', val: b.assetCount + getBuildingSystems(b.name).length, sub: `Active: ${b.activeAssets}`, c: 'text-blue-400 bg-blue-500/10 border-blue-500/20' },
                    ...
                    {getBuildingSystems(b.name).length > 0 && (
                        <div>
                            <p className="text-emerald-500/80 text-xs font-semibold uppercase tracking-wider mb-3 flex items-center gap-1.5">
                                <Monitor size={14} /> Auto-Detected Live Systems
                                <span className="ml-2 text-slate-500 normal-case font-normal">({getBuildingSystems(b.name).length} tracking)</span>
                            </p>
                            <div className="space-y-2">
                                {getBuildingSystems(b.name).map(s => (
                                    <div key={s.id} className="flex items-center justify-between p-3.5 bg-emerald-500/5 border border-emerald-500/20 rounded-xl">
                                        <div>
                                            <p className="text-emerald-50 text-sm font-medium">{s.hostname}</p>
                                            <p className="text-emerald-400/60 text-[11px] font-mono mt-0.5">{s.ip} · {s.mac}</p>
                                        </div>
...
/* ─── Campus totals bar ───────────────────────────────────────── */
function TopBar() {
const total = buildings.reduce((s, b) => s + b.assetCount + getBuildingSystems(b.name).length, 0);
            ...
            <div className="bg-zinc-900/60 border border-zinc-800/60 rounded-2xl overflow-hidden">
                <div className="px-6 py-4 border-b border-zinc-800/40 flex items-center justify-between">
                    <div className="flex items-center gap-2 text-slate-400 text-sm">
                        ...
                        <span>{buildings.length} buildings · {buildings.reduce((s, b) => s + b.assetCount + getBuildingSystems(b.name).length, 0)} assets tracked</span>
                    </div>
                                    ...
                                    return (
                                        <g key={b.id} onClick={() => setSelected(b)} className="cursor-pointer">
                                            ...
                                            <text x={b.x + b.w - 14} y={b.y + 18}
                                                ...
                                                {b.assetCount + getBuildingSystems(b.name).length}
                                            </text>
                            ...
                            {buildings.map(b => {
                                ...
                                return (
                                    <tr key={b.id} onClick={() => setSelected(b)}
                                        ...
                                        <td className="px-5 py-3.5 font-mono font-bold text-slate-200">{b.assetCount + getBuildingSystems(b.name).length}</td>
